...
def value_id(cur, table, column, value):
    cur.execute(
        ...
        f"ON CONFLICT ({column}) DO UPDATE SET {column} = EXCLUDED.{column} "
        f"RETURNING {table[:-1]}_id",
        (value,),
    )
    return cur.fetchone()[0]


def customer_id_by_phone(cur, full_name, phone):
    cur.execute(
        "INSERT INTO customers (full_name, phone) VALUES (%s, %s) "
...
def import_data():
    ...
    with psycopg.connect(pg_dsn()) as conn:
        with conn.cursor() as cur:
            for row in rows:
                customer_id = customer_id_by_phone(
                    cur, row["customer_name"], row["customer_phone"]
                )
                ...
                cur.execute(
                    "INSERT INTO books (isbn, title, genre_id) VALUES (%s, %s, %s) "
                    "ON CONFLICT (isbn) DO UPDATE SET title = EXCLUDED.title, genre_id = EXCLUDED.genre_id "
                    "RETURNING book_id",
                    (row["isbn"], row["book_title"], genre_id),
                )
                ...
                cur.execute(
                    "INSERT INTO sales (sale_number, sold_at, customer_id, employee_id, book_id, quantity, unit_price) "
                    "VALUES (%s, %s, %s, %s, %s, %s, %s) ON CONFLICT (sale_number) DO NOTHING",
                    (
                        row["sale_number"], row["sold_at"], customer_id, employee_id, book_id,
